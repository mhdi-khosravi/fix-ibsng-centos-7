...
class InternetCharge(ChargeWithRules):
    def checkLimits(self,user_obj):
        ...
        for _index in range(user_obj.instances):
            ...
            credit_usage_per_second +=float( effective_rule.cpm) / 60.0 + \
                                       float (effective_rule.cpk) * effective_rule.assumed_kps
    ...
    def calcInstanceRuleCreditUsage(self,user_obj,instance,round_result):
        ...
        if effective_rule.cpm>0:
            timely_credit_used=float(effective_rule.cpm) * (now - user_obj.charge_info.rule_start[instance-1]) / 60
        if effective_rule.cpk>0:
            transfer_credit_used=float(effective_rule.cpk) * (effective_rule.calcRuleTransferUsage(user_obj,instance)) / 1024.0
